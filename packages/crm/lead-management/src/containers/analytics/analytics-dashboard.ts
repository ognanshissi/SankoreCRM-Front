    ...
    <div class="pb-6">
      ...
      <div class="grid grid-cols-4 gap-3 mb-5">
        ...
        >
          <div class="p-4">
            <p class="text-2xl font-bold text-slate-800 tabular-nums">
              {{ fmtPct(funnel()?.qualificationRate) }}
            </p>
          </div>
        </widget-wrapper>
        <widget-wrapper
        ...
        >
          <div class="p-4">
            ...
            <p class="text-xs text-slate-400 mt-0.5">
              {{ fmtPct(funnel()?.lossRate) }} de perte
            </p>
          </div>
        </widget-wrapper>
      </div>
      ...
      <div class="grid grid-cols-2 gap-4 mb-4">
          ...
          <div class="p-4">
            @for (stage of funnelStages(); track stage.label) {
              <div class="flex items-center gap-3 mb-2">
                <span class="text-xs text-slate-500 w-28 shrink-0 truncate">{{
                  stage.label
                }}</span>
                <div
                ...
                <span
                  class="text-xs font-semibold text-slate-700 tabular-nums w-10 text-right"
                  >{{ stage.count }}</span
                >
              </div>
            }
        ...
        <widget-wrapper
          ...
          [hasError]="slaError()"
          (retry)="loadSlaBreaches()"
        >
          @if (slaBreaches().length === 0) {
            <div
              class="p-4 flex flex-col items-center justify-center py-8 text-center"
            >
              <tas-icon
            ...
            <div
              class="divide-y divide-slate-100 max-h-[280px] overflow-y-auto"
            >
            ...
            @if (slaBreaches().length > 10) {
              <div class="p-3 border-t border-slate-100 text-center">
                <span class="text-xs text-slate-400"
                  >+ {{ slaBreaches().length - 10 }} autre(s)</span
                >
              </div>
            }
          }
              ...
              <tbody class="divide-y divide-slate-100">
                @for (a of agents(); track a.agentId) {
                  <tr class="hover:bg-slate-50">
                    ...
                    <td
                      class="px-4 py-2.5 text-right tabular-nums text-green-600"
                    >
                      {{ a.contactedWithinSla ?? 0 }}
                    </td>
                    <td
                      class="px-4 py-2.5 text-right tabular-nums text-amber-600"
                    >
                      {{ a.contactedLate ?? 0 }}
                    </td>
                    <td
                      class="px-4 py-2.5 text-right tabular-nums text-red-500"
                    >
                      {{ a.notContacted ?? 0 }}
                    </td>
                      ...
                      <div class="inline-flex items-center gap-1">
                        ...
                        >
                          <div
                            ...
                            [class]="
                              (a.slaComplianceRate ?? 0) >= 80
                                ? 'bg-green-500'
                                : (a.slaComplianceRate ?? 0) >= 50
                                  ? 'bg-amber-500'
                                  : 'bg-red-400'
                            "
                    ...
                    <td
                      class="px-4 py-2.5 text-right tabular-nums text-slate-500"
                    >
                      {{
                        a.avgFirstContactMinutes != null
                          ? a.avgFirstContactMinutes.toFixed(0) + ' min'
                          : '—'
                      }}
                    </td>
                  </tr>
...
export class AnalyticsDashboard implements OnInit {
  ...
  public loadFunnel(): void {
    ...
    this._analyticsApi
      .getLeadFunnelMetrics(p.from, p.to, this.selectedAgency() || undefined)
      .pipe(
        catchError(() => {
          this.funnelError.set(true);
          return of(null);
        }),
      )
      .subscribe((f) => {
  ...
  public loadAgentPerformance(): void {
    ...
    this._analyticsApi
      .getAgentPerformance(p.from, p.to)
      .pipe(
        catchError(() => {
  ...
  public loadSlaBreaches(): void {
    ...
    this._analyticsApi
      .getLeadSlaBreaches(undefined, this.selectedAgency() || undefined)
      .pipe(
        catchError(() => {
          this.slaError.set(true);
          return of([]);
        }),
      )
      .subscribe((b) => {
        this.slaBreaches.set(b ?? []);
  ...
  private _currentPeriod(): { from: string; to: string } {
    return (
      this.periods.find((p) => p.value === this.selectedPeriod()) ??
      this.periods[0]
    );
  }
